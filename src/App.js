    ...
    <div className="App">
      <Router>
        <Routes>
          ...
          <Route path="/dashboard/*" element={isAuthenticated ? <DashboardLayout /> : <Auth />}>
          ...
          <Route path="/select" element={isAuthenticated ? <Select /> : <Auth />} />
          <Route path="/candidate" element={isAuthenticated ? <Details /> : <Auth />} />
          <Route path="/examiner/:id" element={isAuthenticated ? <Examiner /> : <Auth />} />
          <Route path="/instruction/:id" element={isAuthenticated ? <Instruction /> : <Auth />} />
          <Route path="/result" element={isAuthenticated ? <CandidateResult /> : <Auth />} />
          <Route path="*" element={<div className='h-screen w-screen text-4xl flex justify-center items-center'>404</div>} />
        </Routes>
      </Router>
    </div>
  );
