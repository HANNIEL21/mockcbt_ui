...
import { BsFillGridFill, BsGrid, BsFileText, BsFileTextFill, BsFileEarmarkText, BsFileEarmarkTextFill, BsBriefcase, BsBriefcaseFill } from "react-icons/bs";
    ...
    return (
        <div className="w-72 px-8 py-4 flex flex-col justify-between items-center h-screen bg-white shadow-md">
            <nav className='w-full flex flex-col items-center'>
                <div className=''>
                    <img src={logo} alt="app logo" className='md:h-16 md:w-16 lg:h-24 lg:w-24' />
                </div>
                <h1 className="uppercase font-bold text-2xl text-blue-900">{appName}</h1>
            </nav>
                        ...
                        adminLinks.map((link) => (
                            <li key={link.id}>
                                <Link
                                    ...
                                    to={link.to}
                                    className={`flex items-center gap-3 w-full capitalize font-bold text-left px-4 py-2 rounded-md focus:outline-none ${link.to === selected ? 'text-blue-900 bg-slate-100 shadow-md' : 'text-gray-400'
                                        }`}
                                >
