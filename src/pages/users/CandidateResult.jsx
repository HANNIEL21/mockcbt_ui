    ...
    useEffect(() => {
        const fetch = async () => {
            ...
            try {
                ...
                const res = await axios.get(`${baseApiUrl}/result.php?id="${userDetails?.examno}"`);
        ...
        <main className='p-10'>
            ...
            <section className='flex items-center gap-8 p-8'>
                <div className='h-44 w-44 flex items-center justify-center'>
                    {
                        userDetails?.avatar ? (
                            <img src={`${baseApiUrl}/images/${userDetails?.avatar}`} alt="User Avatar" className='w-full h-full rounded-lg shadow-md' />
                        ) : (
                ...
                <div>
                    ...
                    <p className='font-extrabold capitalize'>course : <span>{userDetails?.course}</span></p>
                    <p className='font-extrabold capitalize'>exam no : <span>{userDetails?.examno}</span></p>
                </div>
            </section>
            <section className=''>
                <Table
