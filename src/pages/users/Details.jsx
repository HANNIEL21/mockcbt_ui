...
const Details = () => {
  ...
  const handleStartExam = async () => {
    ...
    try {
      ...
      const reoccurringExam = res.data.filter((exam) => exam.category === "Multi Choice - Multi Attempt" && exam.group === userDetails?.group);
    ...
    <main className='h-screen w-screen bg-gray-200 flex flex-col'>
      <section className=' w-full h-24 flex justify-between items-center px-10'>
        <div className='flex flex-col items-start'>
          <p>Hello</p>
          {userDetails && <p className='font-bold text-3xl text-blue-900'>{userDetails?.firstname} {userDetails?.lastname}</p>}
        </div>
        <div className='flex gap-4 items-center'>
          <>
            ...
            >
              ...
              {userDetails?.token === "" ? "Click to add token" : userDetails?.token}
            </button>
              ...
              <div className="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                  ...
                  <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <AddToken closeModal={handleOpenModal} user={userDetails?.examno} />
                  </div>
                </div>
              </div>
            )}
      ...
      <section className='flex w-full h-full justify-around items-center'>
        <div onClick={handleStartExam} className='bg-gradient-to-br from-green-800 to-blue-800 p-2 h-[250px] w-[250px] rounded-2xl shadow-md flex items-center justify-center cursor-pointer'>
          <h2 className='text-white font-extrabold text-5xl uppercase bg-transparent'>take <br /> exam</h2>
        </div>
        <div onClick={handleViewResult} className='bg-gradient-to-tr from-blue-800 to-green-800 p-2 h-[250px] w-[250px] rounded-2xl shadow-md flex items-center justify-center cursor-pointer'>
          <h2 className='text-white font-extrabold text-5xl uppercase bg-transparent'>view <br /> results</h2>
        </div>
      </section>
      <section>
      </section>
