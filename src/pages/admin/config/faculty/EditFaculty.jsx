...
const EditFaculty = ({ Id, closeEditFacultyModal }) => {
    ...
    useEffect(() => {
        const fetchFacultyData = async () => {
            ...
            try {
                const response = await axios.get(`${baseApiUrl}/faculty.php?id=${Id}`);
    ...
    const save = async (id) => {
        try {
            setLoading(true);
            setError(null);
            const response = await axios.put(`${baseApiUrl}/faculty.php?id=${id}`, formData);
            if (response.status === 200) {
            ...
            setError('An error occurred while updating Faculty');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 className="text-lg mt-2 leading-6 font-medium text-gray-900" id="modal-title">Edit Faculty</h3>
                        <form className="w-full flex justify-center items-center gap-4 mt-3">
            ...
            <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <button type="button" onClick={closeEditFacultyModal} className="mt-3 w-full inline-flex justify-center rounded-md border shadow-sm px-4 py-2 bg-white text-base font-medium focus:outline-none  sm:mt-0 sm:w-auto sm:text-sm">
                    Cancel
                </button>
            </div>
